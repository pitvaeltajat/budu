...
function Dashboard({ budget, configured, lastFetchedAt }: { budget: DashboardBudget; configured: boolean; lastFetchedAt: Date | null }) {
  const now = new Date();
  ...
  return <><p className="eyebrow">Nykyinen talousarvio</p><h1>{budget.name}</h1><p className="lede">Kuluva kausi verrattuna viime vuoden vastaavaan ajankohtaan.</p>{!configured && <p className="notice">Kitsasta ei ole vielä yhdistetty. Mitään tietoja ei haeta ulkopuolelta.</p>}<section className="summary"><div className="card"><span className="label">Menoarvio</span><strong>{money(planned, budget.currency)}</strong></div><div className="card"><span className="label">Tuloarvio</span><strong>{money(incomePlanned, budget.currency)}</strong></div><div className="card"><span className="label">Toteutuneet menot</span><strong>{money(actual, budget.currency)}</strong></div><div className="card"><span className="label">Menoarviota jäljellä</span><strong className={remaining < 0 ? 'negative' : 'positive'}>{money(remaining, budget.currency)}</strong></div></section><div className="actions"><a className="button secondary" href="/import">Vaihda talousarvio</a><p className="label">{lastFetchedAt ? `Haettu Kitsaasta ${dateTime(lastFetchedAt)}.` : 'Kitsaasta ei ole vielä haettu tietoja.'} Tiedot päivittyvät kerran vuorokaudessa.</p></div><section className="grid" style={{ marginTop: 16 }}><div className="card"><div className="section-head"><h2>Talousarvion kohdat</h2><span className="label">kuluva / viime vuosi</span></div><table><thead><tr><th>Kohta</th><th className="right">Arvio</th><th className="right">Tänä vuonna</th><th className="right">Viime vuonna</th><th className="right">Jäljellä</th></tr></thead><tbody>{budget.lines.map(line => { const used = byCategory.get(line.category) || 0; const prior = previousByCategory.get(line.category) || 0; return <tr key={line.id}><td><CategoryDetail category={line.category} kind={line.kind} account={line.kitsasAccount} currency={budget.currency} plannedCents={line.plannedCents} periodStart={iso(periodStart)} periodEnd={iso(periodEnd)} previousStart={iso(previousStart)} current={itemsFor(current, line.category)} previous={itemsFor(previous, line.category)} /><br/><span className="label">{line.kind === 'INCOME' ? 'Tulo' : 'Meno'}{line.kitsasAccount ? ` · tili ${line.kitsasAccount}` : ''}</span></td><td className="right">{money(line.plannedCents, budget.currency)}</td><td className="right">{money(used, budget.currency)}</td><td className="right">{money(prior, budget.currency)}</td><td className="right">{money(line.plannedCents - used, budget.currency)}</td></tr>; })}</tbody></table></div><div className="card"><div className="section-head"><h2>Viimeisimmät menot</h2><span className="label">kuluva kausi</span></div>{current.filter((item) => item.kind === 'EXPENSE').length ? <table><tbody>{current.filter((item) => item.kind === 'EXPENSE').slice(0, 8).map(item => <tr key={item.id}><td><strong>{item.description}</strong><br/><span className="label">{date(item.occurredOn)} · {item.category || 'Kohdistamaton'}</span></td><td className="right">{money(item.amountCents, budget.currency)}</td></tr>)}</tbody></table> : <div className="empty">Kuluvalle kaudelle ei ole vielä haettu toteutuneita menoja Kitsaasta.</div>}</div></section></>;
}
